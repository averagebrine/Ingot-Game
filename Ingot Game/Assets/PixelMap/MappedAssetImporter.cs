...
//         // check the textures are named with _overlay and _map
//         if (!selectedObjects[0].name.Contains("_overlay") && !selectedObjects[1].name.Contains("_map"))
//         {
//             if(!selectedObjects[0].name.Contains("_map") && !selectedObjects[1].name.Contains("_mapped"))
//             {
//                 Debug.Log("Please make sure the textures are named properly (_overlay and _map)");
//                 return;
//             }
//         }
...
//         // get the path to the textures minus the file name
//         string path = AssetDatabase.GetAssetPath(texture);
//         path = path.Substring(0, path.LastIndexOf('/'));

//         // check we got the names correctly
//         if(!texture.name.Contains("_overlay"))
//         {
...
//         // save the texture in the same path as the originals, with the same name as the original minus the _overlay or _map
//         byte[] bytes = finalTexture.EncodeToPNG();
//         File.WriteAllBytes(path + "/" + texture.name.Substring(0, texture.name.LastIndexOf('_')) + "_source.png", bytes);

//         // reload the asset database
//         AssetDatabase.Refresh();

//         // select the new texture
//         Selection.activeObject = AssetDatabase.LoadAssetAtPath(path + "/" + texture.name.Substring(0, texture.name.LastIndexOf('_')) + "_source.png", typeof(Texture2D));
//     }
// }